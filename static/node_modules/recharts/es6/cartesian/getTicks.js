...
function getTicksEnd(_ref) {
  ...
  var sign =
    len >= 2 ? mathSign(result[1].coordinate - result[0].coordinate) : 1;
  var start, end;
  if (sign === 1) {
  ...
  for (var i = len - 1; i >= 0; i--) {
    var entry = result[i];
    var content = _isFunction(tickFormatter)
...
function getTicksStart(_ref2, preserveEnd) {
  ...
  if (preserveEnd) {
    ...
    var tailContent = _isFunction(tickFormatter)
      ? tickFormatter(tail.value, len - 1)
      : tail.value;
    var tailSize =
      getStringSize(tailContent, {
        fontSize: fontSize,
        letterSpacing: letterSpacing,
      })[sizeKey] + unitSize;
