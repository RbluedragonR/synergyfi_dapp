...
((qrcodegen2) => {
  const _QrCode = class {
    ...
    static encodeSegments(
      ...
      if (
        !(
          _QrCode.MIN_VERSION <= minVersion &&
          minVersion <= maxVersion &&
          maxVersion <= _QrCode.MAX_VERSION
        ) ||
        mask < -1 ||
        mask > 7
      )
      ...
      for (const newEcl of [
        ...
        if (
          boostEcl &&
          dataUsedBits <= _QrCode.getNumDataCodewords(version, newEcl) * 8
        )
          ecl = newEcl;
      }
      let bb = [];
    ...
    drawCodewords(data) {
      if (
        data.length !=
        Math.floor(_QrCode.getNumRawDataModules(this.version) / 8)
      )
        throw new RangeError("Invalid argument");
      let i = 0;
    ...
    static getNumDataCodewords(ver, ecl) {
      return (
        Math.floor(_QrCode.getNumRawDataModules(ver) / 8) -
        _QrCode.ECC_CODEWORDS_PER_BLOCK[ecl.ordinal][ver] *
          _QrCode.NUM_ERROR_CORRECTION_BLOCKS[ecl.ordinal][ver]
      );
    }
...
function QRCodeCanvas(props) {
  ...
  useEffect(() => {
    if (_canvas.current != null) {
      ...
      let cells = qrcodegen_default.QrCode.encodeText(
        value,
