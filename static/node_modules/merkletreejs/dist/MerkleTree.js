...
class MerkleTree extends Base_1.default {
  ...
  constructor(leaves, hashFn = sha256_1.default, options = {}) {
    ...
    if (options.fillDefaultHash) {
      if (typeof options.fillDefaultHash === "function") {
        ...
        buffer_1.Buffer.isBuffer(options.fillDefaultHash) ||
  ...
  getOptions() {
    ...
    return {
      ...
      fillDefaultHash:
        (_b =
          (_a = this.fillDefaultHash) === null || _a === void 0
            ? void 0
  ...
  processLeaves(leaves) {
    ...
    if (this.fillDefaultHash) {
      for (
        ...
        i < Math.pow(2, Math.ceil(Math.log2(this.leaves.length)));
        i++
      ) {
  ...
  static marshalLeaves(leaves) {
    return JSON.stringify(
      leaves.map((leaf) => MerkleTree.bufferToHex(leaf)),
      null,
      2
    );
  }
  ...
  getProofIndicesForUnevenTree(sortedLeafIndices, leavesCount) {
    ...
    for (let layerIndex = 0; layerIndex < depth; layerIndex++) {
      ...
      let proofNodeIndices = siblingIndices.filter(
        (index) => !layerNodes.includes(index)
      );
  ...
  verify(proof, targetNode, root) {
    ...
    for (let i = 0; i < proof.length; i++) {
      ...
      if (this.isBitcoinTree) {
        ...
        buffers[isLeftNode ? "unshift" : "push"](
          buffer_reverse_1.default(data)
        );
  ...
  getLayersAsObject() {
    const layers = this.getLayers().map((layer) =>
      layer.map((value) => this.bufferToHex(value, false))
    );
    const objs = [];
  ...
  isUnevenTree(treeLayers) {
    const depth =
      (treeLayers === null || treeLayers === void 0
        ? void 0
        : treeLayers.length) || this.getDepth();
