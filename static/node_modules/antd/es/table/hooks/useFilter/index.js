...
const collectFilterStates = (columns, init, pos) => {
  ...
  (columns || []).forEach((column, index) => {
    ...
    if (column.filters || "filterDropdown" in column || "onFilter" in column) {
      if ("filteredValue" in column) {
        ...
        if (!("filterDropdown" in column)) {
          filteredValues =
            (_a =
              filteredValues === null || filteredValues === void 0
                ? void 0
                : filteredValues.map(String)) !== null && _a !== void 0
              ? _a
              : filteredValues;
        }
        filterStates.push({
    ...
    if ("children" in column) {
      filterStates = [].concat(
        _toConsumableArray(filterStates),
        _toConsumableArray(
  ...
  rawMergedColumns.flatMap((column) => {
    if ("children" in column) {
      return [column].concat(
        _toConsumableArray(getMergedColumns(column.children || []))
      );
    }
    return [column];
  });
  ...
  const mergedFilterStates = React.useMemo(() => {
    ...
    if (filteredKeysIsAllNotControlled) {
      ...
      return filterStates
        ...
        .map((item) => {
          const col =
            mergedColumns[keyList.findIndex((key) => key === item.key)];
