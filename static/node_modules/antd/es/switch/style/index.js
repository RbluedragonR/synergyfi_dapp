...
const genSwitchSmallStyle = (token) => {
  ...
  const trackPaddingCalc = unit(
    calc(handleSizeSM).add(calc(trackPadding).mul(2)).equal()
  );
    ...
    [componentCls]: {
      [`&${componentCls}-small`]: {
        ...
        [`${componentCls}-loading-icon`]: {
          top: calc(calc(handleSizeSM).sub(token.switchLoadingIconSize))
            .div(2)
            .equal(),
        ...
        [`&${componentCls}-checked`]: {
          ...
          [`${componentCls}-handle`]: {
            insetInlineStart: `calc(100% - ${unit(
              calc(handleSizeSM).add(trackPadding).equal()
            )})`,
          },
        },
  ...
  return {
    [componentCls]: {
      ...
      [`&${componentCls}-checked ${switchHandleCls}`]: {
        insetInlineStart: `calc(100% - ${unit(
          calc(handleSize).add(trackPadding).equal()
        )})`,
      },
      [`&:not(${componentCls}-disabled):active`]: {
...
const genSwitchInnerStyle = (token) => {
  ...
  const switchInnerCls = `${componentCls}-inner`;
  const trackPaddingCalc = unit(
    calc(handleSize).add(calc(trackPadding).mul(2)).equal()
  );
