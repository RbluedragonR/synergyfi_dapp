...
const genResponsiveStyle = (token) => {
  const { componentCls, screenSM, screenMD, marginLG, marginSM, margin } =
    token;
  return {
...
const genBaseStyle = (token) => {
  ...
  return {
    ...
    [`${componentCls}-split ${componentCls}-item`]: {
      borderBlockEnd: `${unit(token.lineWidth)} ${token.lineType} ${
        token.colorSplit
      }`,
      "&:last-child": {
        borderBlockEnd: "none",
...
export const prepareComponentToken = (token) => ({
  ...
  itemPaddingSM: `${unit(token.paddingContentVerticalSM)} ${unit(
