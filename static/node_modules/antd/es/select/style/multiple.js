...
export const getMultipleSelectorUnit = (token) => {
  ...
  const basePadding = token.max(
    token.calc(paddingXXS).sub(lineWidth).equal(),
    0
  );
...
const genSelectionStyle = (token, suffix) => {
  ...
  return {
    [`${componentCls}-multiple${suffixCls}`]: Object.assign(
      Object.assign({}, genOverflowStyle(token)),
      {
        // ========================= Selector =========================
