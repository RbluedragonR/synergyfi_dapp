...
const eth_eip712_util_1 = __importDefault(
  require("../vendor-js/eth-eip712-util")
);
...
class CoinbaseWalletProvider extends eventemitter3_1.EventEmitter {
  constructor(options) {
    ...
    this._subscriptionManager = new SubscriptionManager_1.SubscriptionManager(
      this
    );
    this._relay = null;
    this._addresses = [];
    ...
    this.isCoinbaseWallet =
      (_a = options.overrideIsCoinbaseWallet) !== null && _a !== void 0
        ? _a
        : true;
    this.isCoinbaseBrowser =
      (_b = options.overrideIsCoinbaseBrowser) !== null && _b !== void 0
        ? _b
        : false;
    ...
    const cachedAddresses = this._storage.getItem(
      RelayAbstract_1.LOCAL_STORAGE_ADDRESSES_KEY
    );
    if (cachedAddresses) {
      const addresses = cachedAddresses.split(" ");
      if (addresses[0] !== "") {
        this._addresses = addresses.map((address) =>
          (0, util_1.ensureAddressString)(address)
        );
    ...
    window.addEventListener("message", (event) => {
      ...
      if (event.data.data.action === "dappChainSwitched") {
        ...
        const jsonRpcUrl =
          (_a = event.data.data.jsonRpcUrl) !== null && _a !== void 0
            ? _a
            : this.jsonRpcUrl;
  ...
  get jsonRpcUrl() {
    var _a;
    return (_a = this._storage.getItem(DEFAULT_JSON_RPC_URL)) !== null &&
      _a !== void 0
      ? _a
  ...
  updateProviderInfo(jsonRpcUrl, chainId) {
    ...
    const chainChanged =
      (0, util_1.ensureIntNumber)(chainId) !== originalChainId;
  ...
  async watchAsset(type, address, symbol, decimals, image, chainId) {
    ...
    const result = await relay.watchAsset(
      ...
      decimals,
      image,
      chainId === null || chainId === void 0 ? void 0 : chainId.toString()
    ).promise;
  ...
  async addEthereumChain(
    ...
    if (
      ((_a = res.result) === null || _a === void 0 ? void 0 : _a.isApproved) ===
      true
    ) {
      this.updateProviderInfo(rpcUrls[0], chainId);
    }
    return (
      ((_b = res.result) === null || _b === void 0 ? void 0 : _b.isApproved) ===
      true
    );
  }
  async switchEthereumChain(chainId) {
    ...
    if ((0, Web3Response_1.isErrorResponse)(res)) {
      ...
      if (
        res.errorCode === error_1.standardErrorCodes.provider.unsupportedChain
      ) {
  ...
  async enable() {
    var _a;
    (_a = this.diagnostic) === null || _a === void 0
      ? void 0
      : _a.log(DiagnosticLogger_1.EVENTS.ETH_ACCOUNTS_STATE, {
  ...
  async scanQRCode(match) {
    const relay = await this.initializeRelay();
    const res = await relay.scanQRCode((0, util_1.ensureRegExpString)(match))
      .promise;
    if ((0, Web3Response_1.isErrorResponse)(res)) {
  ...
  async connectAndSignIn(params) {
    ...
    try {
      ...
      if (
        ...
        throw error_1.standardErrors.provider.userRejectedRequest(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (filterPromise !== undefined) {
          filterPromise
            .then((res) =>
              resolve(Object.assign(Object.assign({}, res), { id: request.id }))
            )
  ...
  async _handleAsynchronousMethods(request) {
    ...
    const relay = await this.initializeRelay();
    return relay
      .makeEthereumJSONRPCRequest(request, this.jsonRpcUrl)
      .catch((err) => {
        var _a;
        if (
          err.code === error_1.standardErrorCodes.rpc.methodNotFound ||
          err.code === error_1.standardErrorCodes.rpc.methodNotSupported
        ) {
          (_a = this.diagnostic) === null || _a === void 0
            ? void 0
            : _a.log(DiagnosticLogger_1.EVENTS.METHOD_NOT_IMPLEMENTED, {
  ...
  _ensureKnownAddress(addressString) {
    ...
    if (!this._isKnownAddress(addressString)) {
      (_a = this.diagnostic) === null || _a === void 0
        ? void 0
        : _a.log(DiagnosticLogger_1.EVENTS.UNKNOWN_ADDRESS_ENCOUNTERED);
  ...
  _prepareTransactionParams(tx) {
    ...
    const weiValue =
      tx.value != null
        ? (0, util_1.ensureBN)(tx.value)
        : new bn_js_1.default(0);
    const data = tx.data ? (0, util_1.ensureBuffer)(tx.data) : Buffer.alloc(0);
    const nonce =
      tx.nonce != null ? (0, util_1.ensureIntNumber)(tx.nonce) : null;
    const gasPriceInWei =
      tx.gasPrice != null ? (0, util_1.ensureBN)(tx.gasPrice) : null;
    const maxFeePerGas =
      tx.maxFeePerGas != null ? (0, util_1.ensureBN)(tx.maxFeePerGas) : null;
    ...
    const chainId = tx.chainId
      ? (0, util_1.ensureIntNumber)(tx.chainId)
      : this.getChainId();
    return {
      fromAddress,
      toAddress,
  ...
  async _wallet_addEthereumChain(params) {
    ...
    if (
      ((_a = request.rpcUrls) === null || _a === void 0
        ? void 0
        : _a.length) === 0
    ) {
      return {
    ...
    const success = await this.addEthereumChain(
      chainIdNumber,
      (_b = request.rpcUrls) !== null && _b !== void 0 ? _b : [],
      (_c = request.blockExplorerUrls) !== null && _c !== void 0 ? _c : [],
      request.chainName,
      (_d = request.iconUrls) !== null && _d !== void 0 ? _d : [],
