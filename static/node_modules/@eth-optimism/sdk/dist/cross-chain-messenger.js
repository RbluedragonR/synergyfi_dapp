...
class CrossChainMessenger {
  constructor(opts) {
    ...
    this.populateTransaction = {
      sendMessage: async (message, opts) => {
        if (message.direction === interfaces_1.MessageDirection.L1_TO_L2) {
          return this.contracts.l1.L1CrossDomainMessenger.populateTransaction.sendMessage(
            message.target,
            message.message,
            (opts === null || opts === void 0 ? void 0 : opts.l2GasLimit) ||
      ...
      resendMessage: async (
        ...
        if (this.bedrock) {
          ...
          const legacyL1XDM = new ethers_1.ethers.Contract(
            this.contracts.l1.L1CrossDomainMessenger.address,
            (0, contracts_1.getContractInterface)("L1CrossDomainMessenger"),
      ...
      finalizeMessage: async (message, opts, messageIndex = 0) => {
        ...
        if (this.bedrock) {
          const messageHashV1 = (0, core_utils_1.hashCrossDomainMessagev1)(
            resolved.messageNonce,
            resolved.sender,
            resolved.target,
            resolved.value,
      ...
      depositETH: async (amount, opts, isEstimatingGas = false) => {
        ...
        return this.bridges.ETH.populateTransaction.deposit(
          ...
          contracts_1.predeploys.OVM_ETH,
          amount,
          await getOpts()
        );
      },
                    ...
                    (_b =
                      (_a =
                        opts === null || opts === void 0
                          ? void 0
                          : opts.overrides) === null || _a === void 0
                        ? void 0
                        : _a.from) !== null && _b !== void 0
                      ? _b
                      : from,
                }
              ),
                ...
                from:
                  (_d =
                    (_c =
                      opts === null || opts === void 0
                        ? void 0
                        : opts.overrides) === null || _c === void 0
                      ? void 0
                      : _c.from) !== null && _d !== void 0
                    ? _d
                    : from,
              }
            ),
    ...
    this.l1SignerOrProvider = (0, utils_1.toSignerOrProvider)(
      opts.l1SignerOrProvider
    );
    this.l2SignerOrProvider = (0, utils_1.toSignerOrProvider)(
    ...
    this.l1BlockTimeSeconds =
      (opts === null || opts === void 0 ? void 0 : opts.l1BlockTimeSeconds) !==
      undefined
  ...
  async fpac() {
    if (
      this.contracts.l1.OptimismPortal.address ===
      ethers_1.ethers.constants.AddressZero
    ) {
      return false;
    } else {
  ...
  async getMessagesByTransaction(transaction, opts = {}) {
    var _a, _b;
    await ((_b = (_a = transaction).wait) === null || _b === void 0
      ? void 0
      : _b.call(_a));
  ...
  async toBedrockCrossChainMessage(message, messageIndex = 0) {
    const resolved = await this.toCrossChainMessage(message, messageIndex);
    const { version } = (0, core_utils_1.decodeVersionedNonce)(
      resolved.messageNonce
    );
    if (version.eq(1)) {
      return resolved;
    ...
    if (
      resolved.direction === interfaces_1.MessageDirection.L2_TO_L1 &&
    ...
    return Object.assign(Object.assign({}, resolved), {
      ...
      messageNonce: (0, core_utils_1.encodeVersionedNonce)(
  ...
  async toLowLevelMessage(message, messageIndex = 0) {
    ...
    const encoded = (0, core_utils_1.encodeCrossDomainMessageV1)(
      ...
      try {
        ...
        if (
          !((_a = err === null || err === void 0 ? void 0 : err.message) ===
            null || _a === void 0
            ? void 0
            : _a.toString().includes("CALL_EXCEPTION")) &&
          !((_b = err === null || err === void 0 ? void 0 : err.stack) ===
            null || _b === void 0
            ? void 0
            : _b.toString().includes("execution reverted"))
        ) {
  ...
  async getMessageStatus(
    ...
    const resolved = await this.toCrossChainMessage(message, messageIndex);
    const messageHashV0 = (0, core_utils_1.hashCrossDomainMessagev0)(
      resolved.target,
      resolved.sender,
      resolved.message,
      resolved.messageNonce
    ...
    const messenger =
      resolved.direction === interfaces_1.MessageDirection.L1_TO_L2
        ? this.contracts.l2.L2CrossDomainMessenger
        : this.contracts.l1.L1CrossDomainMessenger;
    const success =
      ...
      if (failure) {
        ...
        if (this.bedrock) {
          ...
          const provenWithdrawal = await this.getProvenWithdrawal(
            (0, utils_1.hashLowLevelMessage)(withdrawal)
          );
  ...
  async waitForMessageStatus(message, status, opts = {}, messageIndex = 0) {
    ...
    while (totalTimeMs < (opts.timeoutMs || Infinity)) {
      ...
      if (resolved.direction === interfaces_1.MessageDirection.L1_TO_L2) {
        ...
        if (
          status === interfaces_1.MessageStatus.UNCONFIRMED_L1_TO_L2_MESSAGE &&
        ...
        if (
          status === interfaces_1.MessageStatus.RELAYED &&
  ...
  async estimateL2MessageGasLimit(message, opts, messageIndex = 0) {
    ...
    const bufferPercent =
      (opts === null || opts === void 0 ? void 0 : opts.bufferPercent) || 20;
    return estimate.mul(100 + bufferPercent).div(100);
  }
  async estimateMessageWaitTimeSeconds(
    message,
    messageIndex = 0,
    fromBlockOrBlockHash,
    toBlockOrBlockHash
    ...
    if (resolved.direction === interfaces_1.MessageDirection.L1_TO_L2) {
      ...
      if (
        ...
        status === interfaces_1.MessageStatus.STATE_ROOT_NOT_PUBLISHED
      ) {
  ...
  async getProvenWithdrawal(withdrawalHash) {
    ...
    const numProofSubmitters = ethers_1.BigNumber.from(
    ...
    for (let i = 0; i < numProofSubmitters; i++) {
      ...
      const game = new ethers_1.ethers.Contract(
        provenWithdrawal.disputeGameProxy,
        (0, utils_1.getContractInterfaceBedrock)("FaultDisputeGame"),
        this.l1SignerOrProvider
      );
      ...
      for (const game of latestGames) {
        try {
          ...
          if (blockNumber.gte(resolved.blockNumber)) {
            matches.push(
              Object.assign(Object.assign({}, game), {
  ...
  async getStateBatchAppendedEventByTransactionIndex(transactionIndex) {
    ...
    let batchEvent = await this.getStateBatchAppendedEventByBatchIndex(
      upperBound
    );
    if (batchEvent === null) {
      return null;
  ...
  async getStateRootBatchByTransactionIndex(transactionIndex) {
    ...
    const stateBatchTransaction =
      await stateBatchAppendedEvent.getTransaction();
    const [stateRoots] =
  ...
  async getMessageProof(message, messageIndex = 0) {
    ...
    const messageSlot = ethers_1.ethers.utils.keccak256(
      ethers_1.ethers.utils.keccak256(
        (0, core_utils_1.encodeCrossDomainMessageV0)(
          resolved.target,
          resolved.sender,
          resolved.message,
          resolved.messageNonce
        ) +
          (0, core_utils_1.remove0x)(
            this.contracts.l2.L2CrossDomainMessenger.address
          )
      ) + "00".repeat(32)
    );
    const stateTrieProof = await (0, utils_1.makeStateTrieProof)(
      (0, utils_1.toJsonRpcProvider)(this.l2Provider),
      resolved.blockNumber,
      this.contracts.l2.OVM_L2ToL1MessagePasser.address,
      messageSlot
    );
    return {
      ...
      stateRootProof: {
        ...
        siblings: (0, utils_1.makeMerkleTreeProof)(
          stateRoot.batch.stateRoots,
          stateRoot.stateRootIndexInBatch
        ),
      },
      stateTrieWitness: (0, core_utils_1.toHexString)(
        rlp.encode(stateTrieProof.accountProof)
      ),
      storageTrieWitness: (0, core_utils_1.toHexString)(
        rlp.encode(stateTrieProof.storageProof)
      ),
    };
  }
  ...
  async sendMessage(message, opts) {
    ...
    if (message.direction === interfaces_1.MessageDirection.L1_TO_L2) {
      return (
        (opts === null || opts === void 0 ? void 0 : opts.signer) ||
        this.l1Signer
      ).sendTransaction(tx);
    } else {
      return (
  ...
  async resendMessage(message, messageGasLimit, opts) {
    return (
      (opts === null || opts === void 0 ? void 0 : opts.signer) || this.l1Signer
    ).sendTransaction(
      await this.populateTransaction.resendMessage(
        message,
        messageGasLimit,
  ...
  async depositETH(amount, opts) {
    return (
      (opts === null || opts === void 0 ? void 0 : opts.signer) || this.l1Signer
    ).sendTransaction(await this.populateTransaction.depositETH(amount, opts));
  }
  async withdrawETH(amount, opts) {
    return (
      (opts === null || opts === void 0 ? void 0 : opts.signer) || this.l2Signer
    ).sendTransaction(await this.populateTransaction.withdrawETH(amount, opts));
  }
