...
exports.migratedWithdrawalGasLimit =
  exports.hashMessageHash =
  exports.hashLowLevelMessage =
    void 0;
...
const hashLowLevelMessage = (message) => {
  return (0, core_utils_1.hashWithdrawal)(
...
const hashMessageHash = (messageHash) => {
  const data = ethers_1.ethers.utils.defaultAbiCoder.encode(
    ["bytes32", "uint256"],
    [messageHash, ethers_1.ethers.constants.HashZero]
  );
...
const migratedWithdrawalGasLimit = (data, chainID) => {
  const dataCost = ethers_1.BigNumber.from(hexDataLength(data)).mul(
  ...
  if (chainID === 420) {
    ...
    const dynamicOverhead = MIN_GAS_DYNAMIC_OVERHEAD_NUMERATOR.mul(1000000).div(
