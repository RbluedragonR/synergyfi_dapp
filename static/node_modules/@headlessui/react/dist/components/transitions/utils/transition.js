import { disposables as f } from "../../../utils/disposables.js";
import { match as d } from "../../../utils/match.js";
import { once as s } from "../../../utils/once.js";
function g(t, ...e) {
  t && e.length > 0 && t.classList.add(...e);
}
function v(t, ...e) {
  t && e.length > 0 && t.classList.remove(...e);
}
function b(t, e) {
  let n = f();
  if (!t) return n.dispose;
  let { transitionDuration: m, transitionDelay: a } = getComputedStyle(t),
    [u, p] = [m, a].map((l) => {
      let [r = 0] = l
        .split(",")
        .filter(Boolean)
        .map((i) => (i.includes("ms") ? parseFloat(i) : parseFloat(i) * 1e3))
        .sort((i, T) => T - i);
      return r;
    }),
    o = u + p;
  if (o !== 0) {
    n.group((r) => {
      r.setTimeout(() => {
        e(), r.dispose();
      }, o),
        r.addEventListener(t, "transitionrun", (i) => {
          i.target === i.currentTarget && r.dispose();
        });
    });
    let l = n.addEventListener(t, "transitionend", (r) => {
      r.target === r.currentTarget && (e(), l());
    });
  } else e();
  return n.add(() => e()), n.dispose;
}
function M(t, e, n, m) {
  let a = n ? "enter" : "leave",
    u = f(),
    p = m !== void 0 ? s(m) : () => {};
  a === "enter" && (t.removeAttribute("hidden"), (t.style.display = ""));
  let o = d(a, { enter: () => e.enter, leave: () => e.leave }),
    l = d(a, { enter: () => e.enterTo, leave: () => e.leaveTo }),
    r = d(a, { enter: () => e.enterFrom, leave: () => e.leaveFrom });
  return (
    v(
      t,
      ...e.base,
      ...e.enter,
      ...e.enterTo,
      ...e.enterFrom,
      ...e.leave,
      ...e.leaveFrom,
      ...e.leaveTo,
      ...e.entered
    ),
    g(t, ...e.base, ...o, ...r),
    u.nextFrame(() => {
      v(t, ...e.base, ...o, ...r),
        g(t, ...e.base, ...o, ...l),
        b(t, () => (v(t, ...e.base, ...o), g(t, ...e.base, ...e.entered), p()));
    }),
    u.dispose
  );
}
export { M as transition };
