...
function createPassword(passwordLength, options, charset) {
  ...
  const [initDigitLength, moreDigits] = getInitialLengthForOption(
    options.digits
  );
  const [initSymbolLength, moreSymbols] = getInitialLengthForOption(
    options.symbols
  );
  const [initLowercaseLength, moreLowercase] = getInitialLengthForOption(
    options.lowercase
  );
  const [initUppercaseLength, moreUppercase] = getInitialLengthForOption(
    options.uppercase
  );
...
function validatePasswordOptions(length, options) {
  ...
  const sum =
    ...
    initUppercaseLength;
  const allExact =
    !moreDigits && !moreSymbols && !moreLowercase && !moreUppercase;
  if (sum > length) {
    throw new Error(
      "Invalid option: Requested characters exceeds expected length"
    );
  }
  if (allExact && sum !== length) {
    throw new Error(
      "Invalid option: Requested less characters than expected length"
    );
  }
}
...
function validatePasswordOption(name, option) {
  ...
  if (typeof option === "number") {
    if (option < 0) {
      throw new Error(
        `Invalid option: ${name} option cannot be a negative number`
      );
    }
    return;
  }
  ...
  throw new Error(
    `Invalid option: ${name} option must be a boolean, number, or object`
  );
}
...
function validateCharsetOption(name, charset) {
  ...
  if (charset.length !== new Set(charset).size) {
    throw new Error(
      `Invalid charset option: ${name} charset contains duplicate characters`
    );
  }
}
/**
...
export function generatePassphrase(length, wordlist, sep = "-") {
  ...
  return getRandomNumbersInRange(length, 0, wordlist.length).reduce(
    (passphrase, value, i) => {
