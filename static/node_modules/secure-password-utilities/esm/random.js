...
function getRandomNumberLessThan(number) {
  ...
  const maxValue = needsTwoBytes
    ? MAXIMUM_TWO_BYTE_VALUE
    : MAXIMUM_ONE_BYTE_VALUE;
  const getRandomNumber = needsTwoBytes
    ? getTwoByteRandomInteger
    : getOneByteRandomInteger;
...
export function randomizeCharacters(characters) {
  ...
  //
  const swapIndices = getRandomNumbersInRange(
    charactersLength,
    0,
    charactersLength
  );
...
export function getRandomValues(numValues, rangeMax = 256) {
  if (numValues < 0) {
    throw new Error(
      "Invalid number of values: number of values to return must be at least 0"
    );
  }
  if (typeof rangeMax !== "number" || rangeMax > 256) {
    throw new Error(
      "Invalid range max: range max must be a number that is at most 256"
    );
  }
  if (numValues === 0) {
