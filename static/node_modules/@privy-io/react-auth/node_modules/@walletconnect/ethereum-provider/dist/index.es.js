...
import {
  getAccountsFromNamespaces as _,
  getSdkError as P,
  isValidArray as f,
...
const T = "wc",
  ...
  M = ["chainChanged", "accountsChanged", "message", "disconnect", "connect"];
...
function L(a) {
  ...
  const c = {
      ...
      methods: i || u,
      events: n || g,
      rpcMap: p({}, t.length ? { [m(t)]: r[m(t)] } : {}),
    },
    h = n?.filter((l) => !g.includes(l)),
  ...
  const w = (h?.length && d?.length) || !s,
    I = {
      chains: [...new Set(w ? c.chains.concat(s || []) : s)],
      methods: [...new Set(c.methods.concat(e != null && e.length ? e : y))],
      events: [...new Set(c.events.concat(o != null && o.length ? o : M))],
      rpcMap: r,
    };
  return { required: t.length ? c : void 0, optional: s.length ? I : void 0 };
}
class C {
  constructor() {
    (this.events = new A()),
  ...
  async connect(t) {
    ...
    try {
          ...
          ((c = this.modal) == null ||
            c.subscribeModal((h) => {
              !h.open &&
                ...
                r(new Error("Connection request reset. Please try again.")));
            })),
          await this.signer
            .connect(
              E(
                p(
                  { namespaces: p({}, s && { [this.namespace]: s }) },
      ...
      this.setChainIds(this.rpc.chains.length ? this.rpc.chains : n),
        this.setAccounts(n),
        this.events.emit("connect", { chainId: v(this.chainId) });
    } catch (e) {
    ...
    try {
      const s = await new Promise(async (e, n) => {
          ...
          this.rpc.showQrModal &&
            ...
            await this.signer
              .authenticate(E(p({}, t), { chains: this.rpc.chains }))
              .then((r) => {
                e(r);
              })
              .catch((r) => {
        ...
        i = s.session;
      if (i) {
        const e = _(i.namespaces, [this.namespace]);
  ...
  registerEventListeners() {
    this.signer.on("session_event", (t) => {
      ...
      this.signer.on("chainChanged", (t) => {
        const s = parseInt(t);
        (this.chainId = s),
          this.events.emit("chainChanged", v(this.chainId)),
          this.persist();
      }),
  ...
  getRpcConfig(t) {
    ...
    const r = e.length ? t?.methods || u : [],
      c = e.length ? t?.events || g : [],
      h = t?.optionalMethods || [],
      d = t?.optionalEvents || [],
  ...
  async initialize(t) {
    if (
      ...
      (this.chainId = this.rpc.chains.length
        ? m(this.rpc.chains)
        : m(this.rpc.optionalChains)),
      (this.signer = await R.init({
      ...
      if (s)
        try {
          this.modal = new s(
            p({ projectId: this.rpc.projectId }, this.rpc.qrModalOptions)
          );
        } catch (i) {
          throw (
  ...
  getRpcUrl(t, s) {
    ...
    return (
      ...
      `${j}?chainId=eip155:${t}&projectId=${s || this.rpc.projectId}`
    );
  }
  async loadPersistedSession() {
    if (this.session)
      try {
        ...
        this.setChainIds(t ? [this.formatChainId(t)] : s?.accounts),
          this.setAccounts(s?.accounts);
      } catch (t) {
